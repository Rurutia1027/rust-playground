...
fn is_valid_user_with_func_pointers<V1, V2, F1, F2>(
    name: &str,
    age: u8,
    c_validator_1: V1,
    c_validator_2: V2,
...
fn main() {
    ...
    // here what's interesting the functions are also supported to passing to another function as a parameters.
    let ret = is_valid_user_with_func_pointers(
        &p1.name,
        p1.age,
        validate_name_v2, // this is the closure 1
        validate_age,     // this is the closure 2
        func_pointer_1,   // this is function pointer 1
        func_pointer_2,   // this is function pointer 2
    );

    println!("\n===this is the ret value of function fn is_valid_user_with_func_pointers<V1, V2, F1, F2> {}", ret);
}

fn func_pointer_1() -> bool {
    println!("func_pointer message");
    true
}

fn func_pointer_2() -> String {
    println!("func_pointer 2 message ");
    "Red".to_string()
}
